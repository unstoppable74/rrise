...
WwiseDataStructure::WwiseDataStructure(const std::filesystem::path& InDirectoryPath, const WwiseDBString* InPlatform)
{
    if (InDirectoryPath.empty())
    {
        return;
    }
    WwiseDBString CollapsedPath = InDirectoryPath;

    IWwiseDirectoryVisitor<WwiseDirectoryVisitor> DirectoryVisitor(new WwiseDirectoryVisitor(InPlatform));
    IWwiseDirectoryVisitor<WwiseDirectoryVisitor>::IterateDirectory(DirectoryVisitor.DirectoryVisitor, InDirectoryPath);
    ...
    if (InPlatform)
    {
        ...
        for (const auto& Platform : ProjectInfoPlatforms)
        {
            if (Platform.Name == *InPlatform)
            {
                ...
                //If the platform path contains a drive, it means that the directory path and platform path are on different drives
