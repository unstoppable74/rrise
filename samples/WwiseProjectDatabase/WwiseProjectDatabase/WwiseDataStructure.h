...
struct WwiseDataStructure
{
	...
	WwiseDataStructure() {}
	WwiseDataStructure(const std::filesystem::path& InDirectoryPath, const WwiseDBString* InPlatform = nullptr);
	~WwiseDataStructure();
...
inline bool WwisePlatformDataStructure::GetRef(RequiredRef& OutRef, const WwiseDBSharedLanguageId& InLanguage, const WwiseDBObjectInfo& InInfo) const
{
	...
	if (InInfo.WwiseShortId != 0)
	{
		if constexpr (!std::is_same_v<RequiredRef, WwiseRefSwitch> && !std::is_same_v<RequiredRef, WwiseRefState>)
		{
