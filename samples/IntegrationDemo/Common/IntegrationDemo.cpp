...
void IntegrationDemo::GetDefaultSettings()
{
	...
	// Register the assert hook in non-debug scenarios; in debug we'll use the built-in assert hook in the soundengine
#if !defined(_DEBUG)
	m_initSettings.pfnAssertHook = IntegrationDemoAssertHook;
#endif
...
// Assert hook used in non-Debug builds. Report that an assert has been tripped, but don't halt the program
void IntegrationDemo::IntegrationDemoAssertHook(const char* in_pszExpression, const char* in_pszFileName, int in_lineNumber)
{
	char assertStr[256];
	snprintf(assertStr, sizeof(assertStr)/sizeof(assertStr[0]), "Assertion triggered in file %s at line %d. Expression: %s\n", in_pszFileName, in_lineNumber, in_pszExpression);
	AKPLATFORM::OutputDebugMsg(assertStr);
	IntegrationDemo::Instance().m_pMenu->GetCurrentPage()->SetErrorMessage(assertStr);
}
...
void IntegrationDemo::Pause()
{
	if ( AK::SoundEngine::IsInitialized() )
	{
		...
		AK::SoundEngine::PostEvent(AK::EVENTS::PAUSE_ALL_GLOBAL, AK_INVALID_GAME_OBJECT);
		AK::SoundEngine::RenderAudio();
#if !defined(AK_IOS) //On iOS, the sound engine automatically handles the suspended state. No need to to call Suspend.
...
void IntegrationDemo::Resume()
{
	...
	{
#if !defined(AK_IOS) //On iOS, the sound engine automatically handles the suspended state. No need to to call WakeupFromSuspend.
