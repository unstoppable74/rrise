        ...
        private void Parse(string[] arguments)
        {
            ...
            try
            {
                for (; i < arguments.Length; i++)
                {
                    ...
                    else if (0 == String.Compare(arguments[i], "-verbosity", true))
                    {
                        UInt32 v;
                        if (i < arguments.Length - 1 && UInt32.TryParse(arguments[++i], out v))
                        {
                            Verbosity = (Severity)(3 - Math.Min(2, v));
                        }
                        else
                            throw new InvalidOperationException("-verbosity: Expected numeric value from 0 (errors only) to 2 (all messages). Default is 2.");
                    }
                }
            }
