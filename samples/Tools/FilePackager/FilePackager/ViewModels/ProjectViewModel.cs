...
namespace FilePackager.ViewModels
{
	...
	public class ProjectViewModel : ViewModelBase
	{
            ...
            using (new AutoCursor(Cursors.Wait))
            {
                ...
                if (System.IO.File.Exists(infoPath))
                {
                    ...
                    if (soundBankInfo != null)
                    {
                        try
                        {
                            NormalizeRootPaths(soundBankInfo, Path.GetDirectoryName(infoPath));
                            ...
                            List<AK.Wwise.InfoFile.FileDescriptorType> fileDescriptorList = new List<AK.Wwise.InfoFile.FileDescriptorType>();

                            fileDescriptorList.AddRange(soundBankInfo.SoundBanks.SoundBank.Cast<AK.Wwise.InfoFile.FileDescriptorType>().Where(x => !string.IsNullOrEmpty(x.GetPath())));

                            List<AK.Wwise.InfoFile.MediaDescriptorType> looseMedia = new List<AK.Wwise.InfoFile.MediaDescriptorType>();
                            List<AK.Wwise.InfoFile.MediaDescriptorType> streamedMedia = new List<AK.Wwise.InfoFile.MediaDescriptorType>();

                            foreach (var bank in soundBankInfo.SoundBanks.SoundBank)
                            {
                                if (bank.Media != null)
                                {
                                    looseMedia.AddRange(bank.Media.File.Cast<AK.Wwise.InfoFile.MediaDescriptorType>().Where(x => !string.IsNullOrEmpty(x.GetPath()) && x.Loose));
                                    streamedMedia.AddRange(bank.Media.File.Cast<AK.Wwise.InfoFile.MediaDescriptorType>().Where(x => !string.IsNullOrEmpty(x.GetPath()) && x.Streaming));
                                }
                            }

                            IEnumerable<AK.Wwise.InfoFile.MediaDescriptorType> looseUnique = looseMedia.Distinct();
                            IEnumerable<AK.Wwise.InfoFile.MediaDescriptorType> streamedUnique = streamedMedia.Distinct().Except(looseUnique);

                            var fileDescriptors = fileDescriptorList.Distinct().Union(looseUnique).Union(streamedUnique);
        ...
        private static void NormalizeRootPaths(AK.Wwise.InfoFile.SoundBanksInfo soundBankInfo, string xmlBaseDir)
        {
            // RootPaths contained in SoundBanksInfo.xml can be either absolute or relative. Normalize them to absolute paths based on the location of the XML file.
            string oldCwd = Directory.GetCurrentDirectory();
            Directory.SetCurrentDirectory(xmlBaseDir);
            soundBankInfo.RootPaths.ProjectRoot = NormalizePath(soundBankInfo.RootPaths.ProjectRoot);
            soundBankInfo.RootPaths.SourceFilesRoot = NormalizePath(soundBankInfo.RootPaths.SourceFilesRoot);
            soundBankInfo.RootPaths.SoundBanksRoot = NormalizePath(soundBankInfo.RootPaths.SoundBanksRoot);
            soundBankInfo.RootPaths.ExternalSourcesInputFile = NormalizePath(soundBankInfo.RootPaths.ExternalSourcesInputFile);
            soundBankInfo.RootPaths.ExternalSourcesOutputRoot = NormalizePath(soundBankInfo.RootPaths.ExternalSourcesOutputRoot);
