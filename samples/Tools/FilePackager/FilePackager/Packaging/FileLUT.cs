...
namespace FilePackager.Packaging
{
    ...
    internal class FileLUT
    {
        ...
        {
			if ( in_fileDesc.GetPath().Length > 0 )
			{
				string szFullPath = System.IO.Path.Combine( m_rootPath, in_fileDesc.GetPath() );
				...
				if ( System.IO.File.Exists( szFullPath ) )
				{
					...
					uint uLanguageID;
					if (!in_mapLanguages.TryGetValue(in_fileDesc.Language, out uLanguageID))
					{
					    uLanguageID = (ushort)AK_INVALID_LANGUAGE_ID;
					}
					System.Diagnostics.Debug.Assert( !m_arFiles.Exists( delegate( IncludedFile in_searchedFile ) { return in_searchedFile.uFileID == in_fileDesc.Id && in_searchedFile.uLanguageID == uLanguageID; } ) );
					{
						...
						m_arFiles.Add( new IncludedFile( in_fileDesc.Id, m_uDefaultBlockAlign, 0, (ulong)fi.Length, (ushort)uLanguageID, szFullPath ) );
					}
					return true;
				}
			}
