    ...
    public class FilePackageGenerator : IProgressNotificationsDispatcher
    {
            ...
            Results results)
        {
            ...
            foreach (AK.Wwise.InfoFile.FileDescriptorType soundbank in descriptors.OfType<AK.Wwise.InfoFile.SoundBanksInfoSoundBanksSoundBank>())
            {
                ...
                {
                    NotifyLogMsg("Missing soundbank: " + soundbank.ShortName + " (" + soundbank.GetPath() + ")", Severity.Warning);
                    results.HasMissingFiles = true;
                }
            }
            banksLUT.Sort();
            
            // Add Streamed files to LUT.
            FileLUT streamsLUT = new FileLUT(globalInfo.SourceFilesRoot, packageInfo.BlockSize, typeof(UInt32));

            foreach (AK.Wwise.InfoFile.FileDescriptorType stream in descriptors.OfType<AK.Wwise.InfoFile.MediaDescriptorType>()
                     .Where(fd => fd.Streaming || fd.Loose))
            {
                if (!streamsLUT.Add(stream, mapLanguageIDs))
                {
                    NotifyLogMsg("Missing streamed or loose media file: " + stream.ShortName + " (" + stream.GetPath() + ")", Severity.Warning);
                    results.HasMissingFiles = true;
                }
            }
            streamsLUT.Sort();
        ...
        static internal Dictionary<string, uint> FindAllLanguages(IEnumerable<AK.Wwise.InfoFile.FileDescriptorType> descriptors )
        {
            ...
            foreach (AK.Wwise.InfoFile.FileDescriptorType streamedFile in descriptors.OfType<AK.Wwise.InfoFile.MediaDescriptorType>()
                .Where(fd => fd.Streaming || fd.Loose))
            {
                string szLanguage = streamedFile.Language;
                if (!string.IsNullOrEmpty(szLanguage) && !mapLanguages.ContainsKey(szLanguage))
                {
                    ++uID;
