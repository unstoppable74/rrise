...
namespace FilePackager.Console
{
    class Application : IApplication
    {
        ...
        public CommandLineViewModel CommandLine { get; set; }
        ...
        public void StartBuild(FilePackager.Packaging.FilePackageGenerator.Context context)
        {
            ...
            buildViewModel.Verbosity = CommandLine.Verbosity;
    ...
    class Program
    {
        static int Main(string[] args)
        {
            ...
            try
            {
                ...
                application.CommandLine = new CommandLineViewModel(args);

                if (!application.CommandLine.IsGenerateMode)
                    throw new InvalidOperationException("Use the \"-generate\" mode at command line.");

                if (!string.IsNullOrEmpty(application.CommandLine.ProjectPath))
                {
                    // Open the project
                    System.Console.WriteLine(string.Format(FilePackager.Console.Properties.Resources.OpeningProject, application.CommandLine.ProjectPath));

                    mainViewModel.OpenProject(application.CommandLine.ProjectPath);
                }
                else
                {
                    ...
                    application.CommandLine.SetupProject(projectViewModel);
                }
