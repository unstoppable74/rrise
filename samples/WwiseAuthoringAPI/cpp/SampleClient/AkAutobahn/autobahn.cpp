...
namespace AK
{
	namespace WwiseAuthoringAPI
	{
		...
		session::session()
		{
			AKPLATFORM::AkCreateEvent(m_sendEvent);
			m_running = false;
		...
		bool session::start(const char* in_uri, unsigned int in_port, disconnectHandler_t disconnectHandler /* = nullptr */)
		{
			...
			{
				m_running = true;
				m_send_thread_started = decltype(m_send_thread_started)();
		...
		void session::stop(const std::string& errorMessage)
		{
			...
			{
				{
					AkAutoLock<CAkLock> lock(m_sendQueueMutex);
					m_sendQueue = decltype(m_sendQueue)();
				}

				// Wake the send thread up, so it terminates.
				AKPLATFORM::AkSignalEvent(m_sendEvent);
				...
				AkAutoLock<CAkLock> lockCalls(m_callsMutex);
				AkAutoLock<CAkLock> lockSubs(m_subreqMutex);
				AkAutoLock<CAkLock> lockUnsubs(m_unsubreqMutex);
		...
		AK::AkFuture<uint64_t> session::join(const std::string& realm, const std::string& method, const std::string& authid,
			const std::string& signature)
		{
			...
			AkAutoLock<CAkLock> lock(m_joinMutex);

			m_session_join = decltype(m_session_join)();

			// Wait for the send thread to start
			// Otherwise send will attempt to notify without the send thready waiting
			bool result = false;
			m_send_thread_started.get_future().get(result);
		...
		bool session::subscribe(const std::string& topic, handler_t handler, const AkJson& options, AK::AkFuture<subscription>& out_future, AkJson& out_jsonError)
		{
			...
			AkAutoLock<CAkLock> lock(m_subreqMutex);
		...
		bool session::unsubscribe(uint64_t subscription_id, AK::AkFuture<result_t>& out_future, AkJson& out_jsonError)
		{
			...
			AkAutoLock<CAkLock> lock(m_unsubreqMutex);
			...
			AkJson& out_jsonError)
		{
			...
			AkAutoLock<CAkLock> lock(m_callsMutex);
		...
		void session::process_subscribed(const wamp_msg_t& msg)
		{
			...
			if (subscribe_request != m_subscribe_requests.end())
			{
				...
				{
					AkAutoLock<CAkLock> lock(m_handlersMutex);
		...
		void session::sendThread()
		{
			...
			while (m_running && m_websocket)
			{
				if (!started)
				{
					m_send_thread_started.set_value(true);
					started = true;
				}
				AKPLATFORM::AkWaitForEvent(m_sendEvent);
