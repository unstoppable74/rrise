						  ...
						  )
{
			...
			m_staticParams.eGenType == AKTONEGENTYPE_SAWTOOTH )
	{
		...
		m_fStartFreqModifier = RandRange( m_staticParams.fStartFreqRandMin, m_staticParams.fStartFreqRandMax, AK_GET_PLUGIN_SERVICE_RNG(in_pSourceFXContext->GlobalContext()));
		...
		if ( m_staticParams.bFreqSweep )
		{
			...
			m_fStopFreqModifier = RandRange( m_staticParams.fStopFreqRandMin, m_staticParams.fStopFreqRandMax, AK_GET_PLUGIN_SERVICE_RNG(in_pSourceFXContext->GlobalContext()));
		}
...
//-----------------------------------------------------------------------------
inline AkReal32 CAkToneGen::RandRange( AkReal32 in_fMin, AkReal32 in_fMax, AK::IAkPluginServiceRNG* in_pRngService )
{
	// Get an integer in range (0,1.)
	if (in_fMin != in_fMax)
	{
		AkReal32 fRandVal = (in_pRngService->RandomSeed() & RAND_MAX) / static_cast<AkReal32>(RAND_MAX);
