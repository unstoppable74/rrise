...
fn main() -> io::Result<()> {
    ...
    println!("cargo:rustc-check-cfg=cfg(wwdebug)");  
    println!("cargo:rustc-check-cfg=cfg(wwrelease)");
    ...
    build
        ...
        .file(wwise_sdk.join("samples").join("SoundEngine").join("Common").join("AkGeneratedSoundBanksResolver.cpp"))  
        .file(wwise_sdk.join("samples").join("SoundEngine").join("Common").join("AkJobWorkerMgr.cpp"))
        .file(
            wwise_sdk.join("samples").join("SoundEngine") .join("Win32").join("AkPlatformProfilerHooks.cpp"))
    ...
    {  
        if let Ok(msvc_includes) = get_msvc_env_paths("INCLUDE") {  
            for path in msvc_includes {  
